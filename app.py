...
def predict():
    try:
        ...
        int_features = df[['ph', 'temperature', 'turbidity']].values.flatten().astype(float)  # Flatten and convert to float
        tank_no= df['tank_no'].values.astype(float)
        ...
        modified_int_features = np.array(int_features).reshape(1,-1)  # Convert to the form [[a, b, c, ...]] for input to the model
